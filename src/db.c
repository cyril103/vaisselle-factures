...
int db_save_company(App *app) {
    ...
    sqlite3_finalize(st);
    g_free(address); g_free(legal);
    return rc;
}

int db_load_company(App *app) {
    sqlite3_stmt *st;
    if (sqlite3_prepare_v2(app->db, "SELECT name,siret,address,email,phone,legal FROM company WHERE id=1", -1, &st, NULL) != SQLITE_OK) return SQLITE_ERROR;
...
int db_add_client(App *app) {
    ...
    if (!name || strlen(name) == 0) return SQLITE_MISUSE;
    sqlite3_stmt *st;
    ...
    sqlite3_bind_text(st,3,gtk_entry_get_text(GTK_ENTRY(app->client_email)),-1,SQLITE_TRANSIENT);
    sqlite3_bind_text(st,4,gtk_entry_get_text(GTK_ENTRY(app->client_phone)),-1,SQLITE_TRANSIENT);
    int rc = sqlite3_step(st) == SQLITE_DONE ? SQLITE_OK : SQLITE_ERROR;
    sqlite3_finalize(st);
    g_free(address);
    return rc;
...
int db_load_clients(App *app) {
    ...
    sqlite3_stmt *st;
    if (sqlite3_prepare_v2(app->db, "SELECT id,name FROM clients ORDER BY name", -1, &st, NULL) != SQLITE_OK) return SQLITE_ERROR;
...
static char *next_invoice_number(sqlite3 *db) {
    ...
    if (sqlite3_prepare_v2(db, "SELECT COALESCE(MAX(id),0)+1 FROM invoices", -1, &st, NULL) == SQLITE_OK) {
        if (sqlite3_step(st) == SQLITE_ROW) seq = sqlite3_column_int(st,0);
        sqlite3_finalize(st);
    }
...
int db_create_invoice(App *app, int client_id) {
    ...
    while (valid && ok) {
        ...
        if (ok) {
            sqlite3_bind_int(line,1,invoice_id); sqlite3_bind_text(line,2,desc,-1,SQLITE_TRANSIENT);
            sqlite3_bind_double(line,3,qty); sqlite3_bind_double(line,4,price); sqlite3_bind_double(line,5,vat);
            sqlite3_bind_double(line,6,ht); sqlite3_bind_double(line,7,tva); sqlite3_bind_double(line,8,ttc);
    ...
    if (ok) {
        ...
        if (ok) {
            sqlite3_bind_double(up,1,total_ht); sqlite3_bind_double(up,2,total_tva); sqlite3_bind_double(up,3,total_ttc); sqlite3_bind_int(up,4,invoice_id);
...
int db_load_invoices(App *app) {
    ...
    if (sqlite3_prepare_v2(app->db, sql, -1, &st, NULL) != SQLITE_OK) return SQLITE_ERROR;
    while (sqlite3_step(st) == SQLITE_ROW) {
        GtkTreeIter it; gtk_list_store_append(app->invoices_store, &it);
        gtk_list_store_set(app->invoices_store, &it, 0, sqlite3_column_int(st,0), 1, sqlite3_column_text(st,1), 2, sqlite3_column_text(st,2), 3, sqlite3_column_text(st,3), 4, sqlite3_column_double(st,4), 5, sqlite3_column_text(st,5), -1);
    }
    sqlite3_finalize(st);
    return SQLITE_OK;
}
...
int db_update_invoice_status(App *app, int invoice_id, const char *status) {
    sqlite3_stmt *st;
    if (sqlite3_prepare_v2(app->db, "UPDATE invoices SET status=? WHERE id=?", -1, &st, NULL) != SQLITE_OK) return SQLITE_ERROR;
